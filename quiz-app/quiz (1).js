...
function buildThemeGrid() {
  ...
  if (window.Auth?.isAdmin?.()) {
    ...
    addBtn.innerHTML = `
      ...
      <div class="theme-card-cta">Créer manuellement →</div>
    `;
    grid.appendChild(addBtn);

    // Bouton 📥 importer JSON
    const importBtn = document.createElement('div');
    importBtn.className = 'theme-card theme-card--import';
    importBtn.innerHTML = `
      <div class="theme-card-icon">📥</div>
      <div class="theme-card-title">Importer un quiz</div>
      <div class="theme-card-cta">Fichier JSON généré par IA →</div>
...
$('theme-grid').addEventListener('click', e => {
  ...
  // Bouton 📥 import
  if (e.target.closest('.theme-card--import')) {
  ...
  // Carte thème
  const card = e.target.closest('.theme-card:not(.theme-card--add):not(.theme-card--import)');
...
/* ── Listener input file (délégué sur le document) ── */
document.addEventListener('change', async e => {
  if (e.target.id !== 'import-json-input') return;
  const file = e.target.files[0];
  if (!file) return;
  e.target.value = '';
  await importJsonQuiz(file);
});

/* ══════════════════════════════════════════
   IMPORT JSON
══════════════════════════════════════════ */
async function importJsonQuiz(file) {
  let json;
  try {
    const text = await file.text();
    json = JSON.parse(text);
  } catch {
    alert('❌ Fichier JSON invalide. Vérifie le format.'); return;
  }

  // Validation de base
  if (!json.theme_id || !json.theme || !json.niveaux) {
    alert('❌ Format incorrect. Le fichier doit contenir : theme_id, theme, icon, color, niveaux.'); return;
  }

  const sb = getClient();
  if (!sb) return;

  const color      = json.color || '#7c3aed';
  const colorLight = hexToRgba(color, 0.15);
  const icon       = json.icon || '❓';

  // Affiche un indicateur
  const importCard = document.querySelector('.theme-card--import');
  const origHtml   = importCard?.innerHTML;
  if (importCard) importCard.querySelector('.theme-card-cta').textContent = '⏳ Import en cours…';

  let imported = 0, errors = 0;

  for (const niveau of NIVEAUX_ORDER) {
    const questions = json.niveaux[niveau];
    if (!questions?.length) continue;

    // Filtre les questions valides
    const valid = questions.filter(q => q.q?.trim() && q.a?.[0]?.trim());
    if (!valid.length) continue;

    const existing = state.allRows.find(r => r.theme_id === json.theme_id && r.niveau === niveau);
    const payload  = {
    ...
    if (error) { console.error(niveau, error.message); errors++; }
    else imported++;
  }

  if (errors > 0 && imported === 0) {
    alert(`❌ Erreur lors de l'import : ${errors} niveau(x) en échec.`);
  } else if (errors > 0) {
    alert(`⚠️ Import partiel : ${imported} niveau(x) importé(s), ${errors} en échec.`);
  } else {
    alert(`✅ Quiz "${json.theme}" importé avec succès ! (${imported} niveau(x))`);
  }
...
function loadEditorQuestions(theme) {
  ...
  editorQuestions = (theme?.niveaux[niveau] || []).map(q => ({ q: q.q, a: [...q.a] }));
  renderEditorQuestions();
}
  ...
  editorQuestions.forEach((q, qi) => {
    ...
    div.innerHTML = `
      <div class="ed-q-header">
        ...
        <button class="ed-q-del q-btn secondary">🗑️</button>
      </div>
      ...
      <div class="ed-answers">
        ...
        ${[1,2,3].map(ai => `<input class="ed-input ed-answer" type="text" placeholder="Mauvaise réponse ${ai}…" value="${escHtml(q.a[ai] || '')}" data-ai="${ai}" />`).join('')}
      </div>
    `;
    div.querySelector('[data-field="q"]').addEventListener('input', e => { editorQuestions[qi].q = e.target.value; });
    div.querySelectorAll('.ed-answer').forEach(inp => {
      inp.addEventListener('input', e => { editorQuestions[qi].a[+e.target.dataset.ai] = e.target.value; });
    });
    div.querySelector('.ed-q-del').addEventListener('click', () => { editorQuestions.splice(qi, 1); renderEditorQuestions(); });
...
$('ed-save').addEventListener('click', async () => {
  ...
  let error;
  if (existing) ({ error } = await sb.from('quizzes').update(payload).eq('id', existing.id));
