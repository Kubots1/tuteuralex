/* ============================================================
   ...
   Recto : mot seul | Verso : traduction + synonymes
   ============================================================ */
'use strict';
...
async function loadVocab() {
  ...
  const { data, error } = await sb
    .from('vocabulaire').select('*').order('created_at', { ascending: false });
...
$v('filtre-niveau').addEventListener('click', e => {
  const pill = e.target.closest('.vocab-pill'); if (!pill) return;
...
function getFilteredWords() {
  return filtreNiveau === 'tous' ? allWords : allWords.filter(w => w.niveau === filtreNiveau);
}
...
$v('vf-save').addEventListener('click', async () => {
  ...
  const syns = $v('vf-synonymes').value.trim();
  const niv  = $v('vf-niveau').value;
  ...
  const { error } = await sb.from('vocabulaire').insert({
    mot, traduction: trad, synonymes: syns || null, niveau: niv
  });
...
function clearForm() {
  $v('vf-mot').value = ''; $v('vf-trad').value = '';
  $v('vf-synonymes').value = ''; $v('vf-niveau').value = 'A1';
}
...
$v('vocab-import-input').addEventListener('change', async e => {
  ...
  if (!list.length) { alert('❌ Aucun mot trouvé.'); return; }
  const valid = list.filter(w => w.mot?.trim() && w.traduction?.trim() && NIVEAUX_ORDER.includes(w.niveau));
  if (!valid.length) { alert('❌ Aucun mot valide.'); return; }
  const sb = getClient(); if (!sb) return;
  ...
  const { error } = await sb.from('vocabulaire').insert(
    valid.map(w => ({
      mot: w.mot.trim(),
      traduction: w.traduction.trim(),
      synonymes: w.synonymes?.trim() || null,
      niveau: w.niveau
    }))
  );
  if (error) alert('❌ Erreur : ' + error.message);
  else { alert(`✅ ${valid.length} mot(s) importé(s) !`); await loadVocab(); }
    ...
    <div class="vocab-admin-list-body">
      ${allWords.map(w => `
        <div class="vocab-admin-item">
          ...
          ${w.synonymes ? `<span class="vocab-admin-syns">${w.synonymes}</span>` : ''}
          <button class="vocab-admin-del" data-id="${w.id}">🗑️</button>
        </div>`).join('')}
    </div>`;
  container.classList.remove('hidden');
  $v('vocab-toggle-list').addEventListener('click', () => container.classList.add('hidden'));
  container.querySelectorAll('.vocab-admin-del').forEach(btn => {
    btn.addEventListener('click', async () => {
      if (!confirm('Supprimer ce mot ?')) return;
      await getClient().from('vocabulaire').delete().eq('id', btn.dataset.id);
      await loadVocab();
    });
  });
}
...
function renderReviewCard() {
  ...
  // Verso — traduction + synonymes séparés
  $v('review-trad').textContent = word.traduction;

  const synsEl = $v('review-synonymes');
  if (word.synonymes) {
    synsEl.innerHTML = `<span class="review-syns-label">Synonymes :</span> ${word.synonymes}`;
    synsEl.classList.remove('hidden');
  } else {
    synsEl.classList.add('hidden');
  }
...
$v('review-good').addEventListener('click', () => { reviewGood++; reviewIndex++; renderReviewCard(); });
$v('review-bad').addEventListener('click',  () => { reviewBad++;  reviewIndex++; renderReviewCard(); });
